...
let printExpr = ref (fun (_: Ast.expr) -> "")
let printPatt = ref (fun (_: Ast.patt) -> "")
...
let rec get_ident = function
    <:expr< $lid:x$ >> -> [x]
  | <:expr< [| $e$ |] >> -> get_ident e
  | <:expr< $h$ ; $t$ >> -> (get_ident h) @ (get_ident t)
  | <:expr< $h$ , $t$ >> -> (get_ident h) @ (get_ident t)
  | <:expr< ($tup:e$) >> -> get_ident e
  | <:expr< $p1$ $p2$ >> -> get_ident p1 @ get_ident p2
  | <:expr< { $rb$ } >> ->
    let lel = 
      let rec collect = function
        | <:rec_binding< $l$ ; $r$ >> -> (collect l) @ (collect r)
        | <:rec_binding< $lab$ = $e$ >> -> [lab, e]
      in
        collect rb
    in
...
let rec get_defined_ident = function
  ...
  | <:patt< $str:_$ >> -> []
  | <:patt< $chr:_$ >> -> []
  | <:patt< $h$ ; $t$ >> -> (get_defined_ident h) @ (get_defined_ident t)
  | <:patt< $h$ , $t$ >> -> (get_defined_ident h) @ (get_defined_ident t)
  | <:patt< [| $p$ |] >> -> get_defined_ident p
  | <:patt< ($p$) >> -> get_defined_ident p
  | <:patt< $uid:_$ >> -> []
  | <:patt< ` $_$ >> -> []
  | <:patt< # $list:_$ >> -> []
  ...
  | <:patt< ? $_$ : ($lid:s$ = $e$) >> -> [s]
  | <:patt< $anti:p$ >> -> [p]
  | <:patt< $list:p$ >> -> failwith "Pattern list not implemented"
  | _ -> [] 
...
EXTEND Gram
  ...
  let_binding: [
    [ "ostap"; doc=OPT doc_name; "("; rule=o_rule; ")" -> 
      ...
      let p = <:patt< $lid:name$ >> in <:binding< $p$ = $rule$ >> 
    ] 
  ];
  ...
  expr: LEVEL "top" [
    [ "ostap"; "("; (p, tree)=o_alternatives; ")" ->
      ...
      let p = <:patt< _ostap_stream >> in
      let mc = <:match_case< $p$ -> $body$ >> in
      let f = <:expr< fun [$mc$] >> in
  ...
  o_rules: [
    [ rules=LIST1 o_rule SEP ";" ->      
      ...
      (List.map	(fun (name, rule) -> let p = <:patt< $lid:name$ >> in <:binding< $p$ = $rule$>>) rules, defs)
    ]
  ];
	...
	let (s, isSema) = 
      ...
      in
	      match tuple with 
		      []  -> (<:expr< () >>, true)
	      | [x] -> (x, false)
	      |  _  -> (<:expr< ($tup:Ast.exCom_of_list tuple$) >>, true) (*TODO or Com*)
	in
		...
		| Some (f, r) -> 
		    let pat = 
		      ...
		      | Some p -> <:patt< $p$ >>
        in
        let mc = <:match_case< $pat$ -> $f$>> in
		    let pfun = <:expr< fun [$mc$] >> in
		    match r with 
		    ...
		    | Some r -> 
			    let pat = 
			      ...
			      | Some p -> <:patt< $p$ >>
			    in
          let mc = <:match_case< $pat$ -> $r$>> in
			    let rfun = <:expr< fun [$mc$] >> in
	      ...
	      then Some (p, n+1)
		      ...
		      let mc      = <:match_case< $patt$ -> $right$ >> in
		      let sfun      = <:expr< fun [$mc$] >> in
		      Some (combi p sfun, n)
            ) p None
	      with 
	        Some (expr, _) -> 
  ...
  o_primary: [
    [ (p, s)=o_reference; args=OPT o_parameters -> 
          match args with 
	       ...
	       let pat = <:patt< _ostap_stream >> in
         let mc = <:match_case< $pat$ -> $body$ >> in
	       (<:expr< fun [$mc$] >>, (Some (Expr.apply s a)))
    ] |
    [ p=UIDENT ->  
            let p' = "get" ^ p in
            let look = <:expr< _ostap_stream # $p'$ >> in
	          let pat = <:patt< _ostap_stream >> in
            let mc = <:match_case< $pat$ -> $look$ >> in
            (<:expr< fun [$mc$] >>, Some (Expr.term p))
    ] |
    [ p=STRING -> 
          let look = <:expr< _ostap_stream # look $str:p$ >> in
	        let pat = <:patt< _ostap_stream >> in
          let mc = <:match_case< $pat$ -> $look$ >> in
          (<:expr<fun [$mc$]>>, Some (Expr.string p))
    ] |
    [ "$"; "("; p=expr; ")" ->
          let look = <:expr< _ostap_stream # look ($p$) >> in
	        let pat = <:patt< _ostap_stream >> in
          let mc = <:match_case< $pat$ -> $look$ >> in
          (<:expr<fun [$mc$]>>, Some (Expr.string (!printExpr p)))
    ] |
...
let _ = Camlp4.Options.add "-tex"  
  (Arg.String 
       ...
       printExpr := (fun e -> string_of_expr e);
			 printPatt := (fun p -> string_of_patt p);
